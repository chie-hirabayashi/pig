...
command = input(
    "新規登録は「A」,廃用は「D」,出産情報の登録は「R」,個体情報の確認は「F」,養豚場全体の生産性の確認は「C」を選択してください。) < "
)
print(command)
...
if command == "R":
    ...
    new_born_number = input("出産頭数を入力してください > ")
...
# if born_num1(リスト[1]) == 0: num1入力関数起動
# elif born_num2(リスト[2]) == 0: num2入力関数起動
